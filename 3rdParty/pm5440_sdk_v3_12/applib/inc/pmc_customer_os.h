...
*     customer board system. Included are the customer sys_handle, read,
...
#ifndef PMC_CUSTOMER_OS_H
#define PMC_CUSTOMER_OS_H
...
#define DIGI_CUST_MMAP_SIZE         0x02000000
...
#undef PMC_64BIT_PCIE


#define CUST_MAX_DIGI_NAME_SIZE 12
...
typedef struct customer_sys_handle_t {
    ...
    char        name_ptr[CUST_MAX_DIGI_NAME_SIZE+1]; /*!< Name of instantiation of device (example value that can be replaced if not needed in customer system) */
...
} customer_sys_handle_t;

PUBLIC pmc_sys_handle_t *customer_sys_handle_create(void);
PUBLIC PMC_ERROR customer_sys_init(pmc_sys_handle_t *sys_handle, void *sys_cfg_ptr, const char *name_ptr);
...
PUBLIC PMC_ERROR customer_event_register(pmc_sys_handle_t *sys_handle, pmc_event_id_t event_id, void *event_cfg);
PUBLIC void customer_event_monitor(pmc_sys_handle_t *sys_handle, void *event_cfg);
...
*
*******************************************************************************/
static INLINE UINT32 customer_sys_reg_read( pmc_sys_handle_t *sys, UINT32 addr ) __attribute((__always_inline__));
static INLINE UINT32 customer_sys_reg_read( pmc_sys_handle_t *sys, UINT32 addr )
{
    UINT32 value;
    volatile UINT32 temp_value;
    customer_sys_handle_t *cust_sys = (customer_sys_handle_t*)sys;


    if (addr == cust_sys->last_write_addr)
    {
        /* when the last write was at the same address, we read another time to
           avoid getting an ordered RD */
        temp_value = *(volatile UINT32 *) ((PMC_UINTPTR)cust_sys->base_addr_ptr + addr);
        value = temp_value;
    }

    value = *(volatile UINT32 *) ((PMC_UINTPTR)cust_sys->base_addr_ptr + addr);
...
*
*******************************************************************************/
static INLINE UINT64 customer_sys_reg_read64( pmc_sys_handle_t *sys, UINT32 addr )
{
    UINT64 value;
    ...
    if (addr == cust_sys->last_write_addr)
    {
        /* when the last write was at the same address, we read another time to 
           avoid getting an ordered RD */
        temp_value = *(volatile UINT64 *) ((PMC_UINTPTR)cust_sys->base_addr_ptr + addr);
        value = temp_value;
    }

    value = *(volatile UINT64 *) ((PMC_UINTPTR)cust_sys->base_addr_ptr + addr);
...
*
*******************************************************************************/
static INLINE void customer_sys_reg_write( pmc_sys_handle_t *sys, UINT32 addr, UINT32 value ) ALWAYS_INLINE;
static INLINE void customer_sys_reg_write( pmc_sys_handle_t *sys, UINT32 addr, UINT32 value )
{
    customer_sys_handle_t *cust_sys = (customer_sys_handle_t*)sys;
    ...
    *(volatile UINT32 *)((PMC_UINTPTR)cust_sys->base_addr_ptr + addr) = value;

    cust_sys->last_write_addr = addr;
}

#ifdef PMC_64BIT_PCIE
/*******************************************************************************
* customer_sys_reg_write64
* ______________________________________________________________________________
*
* DESCRIPTION:
    Base customer write.
...
*
*******************************************************************************/
static INLINE void customer_sys_reg_write64( pmc_sys_handle_t *sys, UINT32 addr, UINT64 value64 )
{
    customer_sys_handle_t *cust_sys = (customer_sys_handle_t*)sys;
    ...
    *(volatile UINT64 *)((PMC_UINTPTR)cust_sys->base_addr_ptr + addr) = value64;
...
/*******************************************************************************
* customer_sys_burst_read
* ______________________________________________________________________________
*
* DESCRIPTION:
*   Base customer burst read
*
* INPUTS:
*   sys    - System handle
...
*
*******************************************************************************/
static INLINE void customer_sys_burst_read(pmc_sys_handle_t *sys, UINT32 addr, UINT32 len, UINT32 *data )
{
#ifdef PMC_64BIT_PCIE
    if (len == 0) {
        return;
    ...
    if (addr & 0x4) {
        *data = customer_sys_reg_read( sys, addr );
        addr += 4;
        data++;
        len --;
    }
    while (len >= 2)
    {
        ...
        *(UINT64 *)data = customer_sys_reg_read64( sys, addr );
        data += 2;
        addr += 8;
        len  -= 2;
    }
...
/*******************************************************************************
* customer_sys_burst_write
* ______________________________________________________________________________
*
* DESCRIPTION:
*   Base customer burst write
*
* INPUTS:
*   sys    - System handle
...
*
*******************************************************************************/
static INLINE void customer_sys_burst_write(pmc_sys_handle_t *sys, UINT32 addr, UINT32 len, UINT32 *data )
{
#ifdef PMC_64BIT_PCIE
    if (len == 0) {
        return;
    ...
    if (addr & 0x4) {
        customer_sys_reg_write( sys, addr, *data );
        addr += 4;
        data++;
        len --;
    }
    ...
    {
        /* might not work on big endian architecture ? */
        customer_sys_reg_write64( sys, addr, *(UINT64 *)data );
        data += 2;
        addr += 8;
        len  -= 2;
    }
...
/*******************************************************************************
* customer_sys_reg_read_modify_write
* ______________________________________________________________________________
*
* DESCRIPTION:
*   Base customer read modify write
*
* INPUTS:
*   sys     - System handle
*   addr    - 32-bit device address to write
...
*
*******************************************************************************/
static INLINE UINT32 customer_sys_reg_read_modify_write( pmc_sys_handle_t *sys, 
                                                    UINT32 addr,
                                                    UINT32 mask,
                                                    UINT32 value,
...
{
    UINT32 regval;
    regval = customer_sys_reg_read(sys,addr);
    if (oldval != NULL)
    {
...
*
*******************************************************************************/
static INLINE void customer_sys_mem_write(pmc_sys_handle_t *sys, UINT32 addr, UINT32 len, UINT8 *data_ptr )
{
    customer_sys_handle_t *cust_sys = (customer_sys_handle_t*)sys;
    UINT8 *dst_ptr;

    dst_ptr = (UINT8 *)cust_sys->base_addr_ptr + addr;

    PMC_MEMCPY(dst_ptr, data_ptr, len);
}

/*******************************************************************************
*  customer_sys_wb_invalidate_cache
...
*     None
*
*  NOTES:         This is used when communicating with the firmware queue and
*                 will be used when reading the PMON data uploaded by firmware.
*
*******************************************************************************/
static INLINE void customer_sys_wb_invalidate_cache( void *virt_addr, UINT32 size )
{
}

#else

/* If not inline, then define in pmc_customer.c */
UINT32 customer_sys_reg_read( pmc_sys_handle_t *sys, UINT32 addr );

void customer_sys_reg_write( pmc_sys_handle_t *sys, UINT32 addr, UINT32 value );

void customer_sys_burst_read(pmc_sys_handle_t *sys, UINT32 addr, UINT32 len, UINT32 *value );

void customer_sys_burst_write(pmc_sys_handle_t *sys, UINT32 addr, UINT32 len, UINT32 *value );

void customer_sys_mem_write(pmc_sys_handle_t *sys, UINT32 addr, UINT32 len, UINT8 *data_ptr );
