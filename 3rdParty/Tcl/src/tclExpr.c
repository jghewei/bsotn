...
static int
ExprParseString(Tcl_Interp *interp, char *string, Value *valuePtr)
{
    register char c;
...
ExprLex(Tcl_Interp *interp, register ExprInfo *infoPtr, register Value *valuePtr)
{
	...
	case '.':
		...
		if (term2 == infoPtr->expr) {
                  sprintf (interp->result, "poorly-formed floating-point value");
		    return TCL_ERROR;
		}
		valuePtr->type = TYPE_DOUBLE;
		infoPtr->expr = term2;
...
static int
ExprGetValue(Tcl_Interp *interp, register ExprInfo *infoPtr, 
             int prec, Value *valuePtr)
{
    Interp *iPtr = (Interp *) interp;
    Value value2;			/* Second operand for current
					 * operator.  */
    int operat;	        		/* Current operator (either unary
					 * or binary). */
	...
	if ((operat < MULT) || (operat >= UNARY_MINUS)) {
	    if ((operat == END) || (operat == CLOSE_PAREN)) {
		result = TCL_OK;
		goto done;
	    } else {
		goto syntaxError;
	...
	if ((operat == AND) || (operat == OR) || (operat == QUESTY)) {
	    ...
	    if (((operat == AND) && !valuePtr->intValue)
		    || ((operat == OR) && valuePtr->intValue)) {
		iPtr->noEval++;
		result = ExprGetValue(interp, infoPtr, precTable[operat],
			&value2);
		iPtr->noEval--;
	    } else if (operat == QUESTY) {
	...
	switch (operat) {
	    ...
	    default:
              sprintf(interp->result, "unknown operator in expression");
		result = TCL_ERROR;
		goto done;
	}
	...
	switch (operat) {
		...
		if (valuePtr->type == TYPE_INT) {
			...
			divideByZero:
                      sprintf(interp->result, "divide by zero");
			result = TCL_ERROR;
			goto done;
		    }
		    valuePtr->intValue /= value2.intValue;
	    ...
	    case COLON:
              sprintf(interp->result, "can't have : operator without ? first");
		result = TCL_ERROR;
		goto done;
	}
    }
    ...
    Tcl_AppendResult(interp, "can't use ", (badType == TYPE_DOUBLE) ?
	    ...
	    " as operand of \"", operatorStrings[operat], "\"",
	    (char *) NULL);
    result = TCL_ERROR;
    goto done;
}
...
static int
ExprTopLevel(Tcl_Interp *interp, char *string, Value *valuePtr)
{
    ExprInfo info;
    int result;
...
int
Tcl_ExprLong(Tcl_Interp *interp, char *string, long *ptr)
{
    Value value;
    int result;
	...
	if (value.type == TYPE_INT) {
	    ...
	    *ptr = value.doubleValue;
          ...
          sprintf(interp->result, "expression didn't have numeric value");
	    result = TCL_ERROR;
	}
    }
...
int
Tcl_ExprDouble(Tcl_Interp *interp, char *string, double *ptr)
{
    Value value;
    int result;
...
int
Tcl_ExprBoolean(Tcl_Interp *interp, char *string, int *ptr)
{
    Value value;
    int result;
...
int
Tcl_ExprString(Tcl_Interp *interp, char *string)
{
    Value value;
    int result;
