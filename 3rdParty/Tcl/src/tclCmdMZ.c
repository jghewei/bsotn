 ...
 *
 * $Id: tclCmdMZ.c,v 1.1.1.1 2001/04/20 15:03:06 karl Exp $
 */
 ...
 */
	...
	/* ARGSUSED */
int
Tcl_RegexpCmd(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
    int noCase = 0;
    int indices = 0;
    regexp *regexpPtr;
	...
	*dst = 0;
    ...
    argc -= 2;
    if (argc > NSUBEXP) {
      sprintf (interp->result, "too many substring variables");
	return TCL_ERROR;
    }
    for (i = 0; i < argc; i++) {
	char *result, info[50];
 ...
 */
	...
	/* ARGSUSED */
int
Tcl_RegsubCmd(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
    int noCase = 0, all = 0;
    regexp *regexpPtr;
 ...
 */
	...
	/* ARGSUSED */
int
Tcl_RenameCmd(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
    ...
    hPtr = Tcl_CreateHashEntry(&iPtr->commandTable, argv[2], &newnew);
 ...
 */
	...
	/* ARGSUSED */
int
Tcl_ReturnCmd(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
    (void) dummy;
    if (argc > 2) {
	Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0],
 ...
 */
	...
	/* ARGSUSED */
int
Tcl_ScanCmd(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
...
#   define MAX_FIELDS 20
	...
	if (numFields == MAX_FIELDS) {
          sprintf (interp->result, "too many fields to scan");
	    return TCL_ERROR;
	}
 ...
 */
	...
	/* ARGSUSED */
int
Tcl_SplitCmd(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
    char *splitChars;
    register char *p, *p2;
    char *elementStart;
    ...
    if (argc == 2) {
      ...
      splitChars = strdup(argv[2]);
      // sprintf (splitChars, "%s", argv[2]);
    } else {
	Tcl_AppendResult(interp, "wrong # args: should be \"", argv[0],
 ...
 */
	...
	/* ARGSUSED */
int
Tcl_StringCmd(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
    int length;
    register char *p, c;
    int match;
	...
	trim:
	if (argc == 4) {
          trimChars = strdup(argv[3]);
          // sprintf (trimChars, "%s", argv[3]);
	} else if (argc == 3) {
          trimChars = strdup(" \t\n\r");
          // sprintf (trimChars, " \t\n\r");
	} else {
	...
	return TCL_OK;
	    ...
	    && (length > 4)) {
	left = 1;
	sprintf (argv[1], "trimleft");
	goto trim;
    } else if ((c == 't') && (strncmp(argv[1], "trimright", length) == 0)
	    && (length > 4)) {
	right = 1;
	sprintf (argv[1], "trimright");
	goto trim;
    } else {
 ...
 */
	...
	/* ARGSUSED */
int
Tcl_TraceCmd(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
    char c;
    int length;
    (void) dummy;
 ...
 */
	...
	/* ARGSUSED */
static char *
TraceVarProc(ClientData clientData, Tcl_Interp *interp, 
...
#define STATIC_SIZE 199
	...
	*p = ' ';
	...
	if (code != TCL_OK) {
          sprintf (result, "access disallowed by trace command");
 ...
 */
	...
	/* ARGSUSED */
int
Tcl_WhileCmd(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
    int result, value;
    (void) dummy;
