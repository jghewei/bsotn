 ...
 *
 * $Id: tclHash.c,v 1.1.1.1 2001/04/20 15:03:06 karl Exp $
 */
...
Tcl_DeleteHashEntry(Tcl_HashEntry *entryPtr)
{
	...
	*entryPtr->bucketPtr = entryPtr->nextPtr;
	...
	for (prevPtr = *entryPtr->bucketPtr; ; prevPtr = prevPtr->nextPtr) {
	    if (prevPtr == NULL) {
              fprintf (stderr, "malformed bucket chain Tcl_DeleteHashEntry");
              fflush (stderr);
              panic();
	    }
...
OneWordCreate(Tcl_HashTable *tablePtr, register const char *key, int *newPtr)
{
    ...
    *newPtr = 1;
    ...
    hPtr->nextPtr = *hPtr->bucketPtr;
    hPtr->clientData = 0;
    sprintf (hPtr->key.oneWordValue, "%s", key);
    *hPtr->bucketPtr = hPtr;
    tablePtr->numEntries++;
...
BogusFind(Tcl_HashTable *tablePtr, const char *key)
{
    (void) tablePtr;
    (void) key;
    fprintf (stderr, "called Tcl_FindHashEntry on deleted table");
    fflush (stderr);
    panic();
    return NULL;
}
...
BogusCreate(Tcl_HashTable *tablePtr, const char *key, int *newPtr)
{
    ...
    (void) key;
    (void) newPtr;
    fprintf (stderr, "called Tcl_CreateHashEntry on deleted table");
    fflush (stderr);
    panic();
    return NULL;
}
