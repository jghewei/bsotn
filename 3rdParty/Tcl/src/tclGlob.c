 ...
 *
 * $Id: tclGlob.c,v 1.1.1.1 2001/04/20 15:03:06 karl Exp $
 */
...
static void		AppendResult _ANSI_ARGS_((Tcl_Interp *interp,
			    char *dir, const char *separator, char *name,
			    int nameLength));
...
static void
AppendResult(Tcl_Interp *interp, char *dir, const char *separator, 
...
#define STATIC_SIZE 200
	...
	if (closeBrace == NULL) {
	    ...
	    sprintf (interp->result, "unmatched open-brace in file name");
	    return TCL_ERROR;
	}
...
char *
Tcl_TildeSubst(Tcl_Interp *interp, const char *name)
{
 ...
 */
	...
	/* ARGSUSED */
int
Tcl_GlobCmd(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
	...
	if (*thisName == '/') {
          sprintf (baseDir, "/");
	    result = DoGlob(interp, baseDir, thisName+1);
	} else {
          baseDir[0] = '\0';
	    result = DoGlob(interp, baseDir, thisName);
	}
	if (result != TCL_OK) {
	    return result;
	}
