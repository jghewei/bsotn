 ...
 *
 * $Id: tclEnv.c,v 1.1.1.1 2001/04/20 15:03:06 karl Exp $
 */
...
static int		FindVariable _ANSI_ARGS_((CONST char *name,
			    int *lengthPtr));
/*
void			TCL_setenv _ANSI_ARGS_((CONST char *name,
			    CONST char *value));
*/
void			TCL_unsetenv _ANSI_ARGS_((CONST char *name));

/*
 *----------------------------------------------------------------------
 *
...
static int
FindVariable(const char *name, int *lengthPtr)
{
    int i;
...
void
TCL_setenv(const char *name, const char *value)
{
 ...
 *    const char *name2;	Name of variable being modified, or
 ...
 */
	...
	/* ARGSUSED */
static char *
EnvTraceProc(ClientData clientData, Tcl_Interp *interp, const char *name1, 
             const char *name2, int flags)
{
    (void) clientData;
    (void) name1;
	...
	TCL_setenv(name2, Tcl_GetVar2(interp, "env", name2, TCL_GLOBAL_ONLY));
    }
