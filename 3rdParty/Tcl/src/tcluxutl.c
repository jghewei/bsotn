 ...
 *
 * $Id: tcluxutl.c,v 1.1.1.1 2001/04/20 15:03:06 karl Exp $
 */
...
Tcl_DetachPids(int numPids, int *pidPtr)
{
	...
	for (waitPtr = waitTable, count = waitTableUsed;
		...
		*waitPtr = waitTable[waitTableUsed-1];
        ...
        fprintf (stderr, "Tcl_Detach couldnt find process");
        fflush (stderr);
	panic();
 ...
 *	is up to the caller to free this array when it isnt needed
 *	anymore.  If inPipePtr is non-NULL, *inPipePtr is filled in
 *	with the file id for the input pipe for the pipeline (if any):
 *	the caller must eventually close this file.  If outPipePtr
 *	isnt NULL, then *outPipePtr is filled in with the file id
 *	for the output pipe from the pipeline:  the caller must close
 *	this file.  If errFilePtr isnt NULL, then *errFilePtr is filled
...
int
Tcl_CreatePipeline(Tcl_Interp *interp, int argc, char **argv, 
                   int **pidArrayPtr, int *inPipePtr,
                   int *outPipePtr, int *errFilePtr)
{
    ...
    int lastOutputId = -1;	/* Write file id for output from last command
				 ...
				 * -1 means use stdout. */
    int pipeIds[2];		/* File ids for pipe thats being created. */
	...
	*errFilePtr = -1;
     ...
     * of the pipeline.  Count the number of distinct processes (its the
	...
	if ((argv[i][0] == '|') && ((argv[i][1] == 0))) {
	    if ((i == (lastBar+1)) || (i == (argc-1))) {
              sprintf (interp->result, "illegal use of | in command");
		return -1;
	    }
	    lastBar = i;
	    cmdCount++;
	...
	if (i >= (argc-1)) {
	    Tcl_AppendResult(interp, "cant specify \"", argv[i],
    ...
    if (argc == 0) {
      sprintf (interp->result,  "didnt specify command to execute");
	return -1;
    }
...
#	define TMP_STDERR_NAME "/tmp/tcl.err.XXXXXX"
	...
	*errFilePtr = open(errName, O_RDONLY, 0);
	...
	if (pid == 0) {
		...
		char err[100];
		sprintf (err, "forked process couldnt set up input/output\n");
...
int
TclGetOpenFile(Tcl_Interp *interp, const char *string, OpenFile **filePtrPtr)
{
