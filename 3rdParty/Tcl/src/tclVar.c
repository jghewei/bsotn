 ...
 *
 * $Id: tclVar.c,v 1.1.1.1 2001/04/20 15:03:06 karl Exp $
 */
...
const char *noSuchVar =	"no such variable";
const char *isArray =		"variable is array";
const char *needArray =	"variable isn't array";
const char *noSuchElement =	"no such element in array";
const char *traceActive =	"trace is active on variable";
...
static  char *		CallTraces _ANSI_ARGS_((Interp *iPtr, Var *arrayPtr,
			    Tcl_HashEntry *hPtr, const char *part1, 
                            const char *part2, int flags));
static void		DeleteSearches _ANSI_ARGS_((Var *arrayVarPtr));
static void		DeleteArray _ANSI_ARGS_((Interp *iPtr, const char *arrayName,
...
char *
Tcl_GetVar2(Tcl_Interp *interp, const char *part1, const char *part2, 
            int flags)
{
    Tcl_HashEntry *hPtr;
    Var *varPtr;
...
char *
Tcl_SetVar(Tcl_Interp *interp, char *varName, 
...
char *
Tcl_SetVar2(Tcl_Interp *interp, const char *part1, const char *part2, 
            const char *newValue, int flags)
{
    Tcl_HashEntry *hPtr;
    ...
    if ((flags & TCL_GLOBAL_ONLY) || (iPtr->varFramePtr == NULL)) {
	hPtr = Tcl_CreateHashEntry(&iPtr->globalTable, part1, &newnew);
    } else {
	...
	hPtr = Tcl_CreateHashEntry(varPtr->value.tablePtr, part2, &newnew);
    }
...
int
Tcl_UnsetVar2(Tcl_Interp *interp, const char *part1, const char *part2, int flags)
{
...
int
Tcl_TraceVar2(Tcl_Interp *interp, const char *part1, const char *part2, 
              int flags, Tcl_VarTraceProc *proc, ClientData clientData)
{
	...
	hPtr = Tcl_CreateHashEntry(&iPtr->varFramePtr->varTable, part1, &newnew);
    }
    if (!newnew) {
	varPtr = (Var *) Tcl_GetHashValue(hPtr);
	if (varPtr->flags & VAR_UPVAR) {
...
void
Tcl_UntraceVar(Tcl_Interp *interp, char *varName, int flags, 
               Tcl_VarTraceProc *proc, ClientData clientData)
{
    register char *p;
...
void
Tcl_UntraceVar2(Tcl_Interp *interp, const char *part1, const char *part2, 
                int flags, Tcl_VarTraceProc *proc, ClientData clientData)
{
    register VarTrace *tracePtr;
    VarTrace *prevPtr;
...
ClientData
Tcl_VarTraceInfo(Tcl_Interp *interp, char *varName, int flags, 
...
ClientData
Tcl_VarTraceInfo2(Tcl_Interp *interp, const char *part1, const char *part2, 
 ...
 */
	...
	/* ARGSUSED */
int
Tcl_SetCmd(ClientData dummy, register Tcl_Interp *interp, 
           int argc, char **argv)
{
    (void) dummy;
    if (argc == 2) {
 ...
 */
	...
	/* ARGSUSED */
int
Tcl_UnsetCmd(ClientData dummy, register Tcl_Interp *interp, 
             int argc, char **argv)
{
    int i;
    (void) dummy;
 ...
 */
	...
	/* ARGSUSED */
int
Tcl_AppendCmd(ClientData dummy, register Tcl_Interp *interp, 
              int argc, char **argv)
{
    int i;
 ...
 */
	...
	/* ARGSUSED */
int
Tcl_LappendCmd(ClientData dummy, register Tcl_Interp *interp, 
               int argc, char **argv)
{
    int i;
    char *result = NULL;		/* (Initialization only needed to keep
 ...
 */
	...
	/* ARGSUSED */
int
Tcl_ArrayCmd(ClientData dummy, register Tcl_Interp *interp, 
             int argc, char **argv)
{
    int length;
    char c;
 ...
 */
	...
	/* ARGSUSED */
int
Tcl_GlobalCmd(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
    Var *varPtr, *gVarPtr;
    register Interp *iPtr = (Interp *) interp;
	...
	hPtr = Tcl_CreateHashEntry(&iPtr->globalTable, *argv, &newnew);
	if (newnew) {
	    gVarPtr = NewVar(0);
	...
	hPtr2 = Tcl_CreateHashEntry(&iPtr->varFramePtr->varTable, *argv, &newnew);
	if (!newnew) {
	    Var *varPtr;
 ...
 */
	...
	/* ARGSUSED */
int
Tcl_UpvarCmd(ClientData dummy, Tcl_Interp *interp, int argc, char **argv)
{
    register Interp *iPtr = (Interp *) interp;
    int result;
    CallFrame *framePtr;
    ...
    while (argc > 0) {
        hPtr = Tcl_CreateHashEntry(upVarTablePtr, argv[0], &newnew);
        if (newnew) {
            upVarPtr = NewVar(0);
...
void
TclDeleteVars(Interp *iPtr, Tcl_HashTable *tablePtr)
{
    Tcl_HashSearch search;
    Tcl_HashEntry *hPtr;
    register Var *varPtr;
...
static char *
CallTraces(Interp *iPtr, register Var *arrayPtr, Tcl_HashEntry *hPtr, 
...
static ArraySearch *
ParseSearchId(Tcl_Interp *interp, Var *varPtr, char *varName, char *string)
{
    char *end;
    int id;
    ArraySearch *searchPtr;
...
static void
DeleteSearches(register Var *arrayVarPtr)
{
    ArraySearch *searchPtr;
...
static void
DeleteArray(Interp *iPtr, const char *arrayName, Var *varPtr, int flags)
{
    Tcl_HashSearch search;
	...
	if (elPtr->flags & VAR_SEARCHES_POSSIBLE) {
          fprintf (stderr, "DeleteArray found searches on array alement!");
          fflush (stderr);
          panic();
	}
	ckfree((char *) elPtr);
...
static void
VarErrMsg(Tcl_Interp *interp, const char *part1, const char *part2, 
