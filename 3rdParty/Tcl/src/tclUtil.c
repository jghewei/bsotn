 ...
 *
 * $Id: tclUtil.c,v 1.1.1.1 2001/04/20 15:03:06 karl Exp $
 */
...
const char *tclRegexpError = NULL;
...
int
TclFindElement(Tcl_Interp *interp, register char *list, char **elementPtr, 
...
void
TclCopyAndCollapse(int count, register char *src, register char *dst)
{
    register char c;
    int numRead;
...
int
Tcl_ScanElement(const char *string, int *flagPtr)
{
...
int
Tcl_ConvertElement(register const char *src, char *dst, int flags)
{
    register char *p = dst;
...
int
Tcl_StringMatch(register char *string, register char *pattern)
{
    char c2;
...
void
Tcl_SetResult(Tcl_Interp *interp, const char *string, Tcl_FreeProc *freeProc)
{
...
Tcl_AppendResult(Tcl_Interp *interp, ...)
{
    ...
    newSpace = 0;
    while (1) {
	stringv = va_arg(argList, char *);
	if (stringv == NULL) {
	    break;
	}
	newSpace += strlen(stringv);
    }
    va_end(argList);
	...
	strcpy(iPtr->appendResult + iPtr->appendUsed, stringv);
	iPtr->appendUsed += strlen(stringv);
    }
    va_end(argList);
}

...
void
Tcl_AppendElement(Tcl_Interp *interp, char *string, int noSep)
{
    register Interp *iPtr = (Interp *) interp;
    int size, flags;
    char *dst;
...
static void
SetupAppendBuffer(register Interp *iPtr, int newSpace)
{
    int totalSpace;
	...
	newnew = (char *) ckalloc((unsigned) totalSpace);
	strcpy(newnew, iPtr->result);
	if (iPtr->appendResult != NULL) {
	    ckfree(iPtr->appendResult);
	}
	iPtr->appendResult = newnew;
...
regexp *
TclCompileRegexp(Tcl_Interp *interp, char *string)
{
    register Interp *iPtr = (Interp *) interp;
    int i, length;
    regexp *result;
