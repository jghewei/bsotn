 ...
 *
 * $Id: tclBasic.c,v 1.1.1.1 2001/04/20 15:03:06 karl Exp $
 *
 */
...
void
Tcl_CreateCommand(Tcl_Interp *interp, const char *cmdName, Tcl_CmdProc *proc, 
                  ClientData clientData, Tcl_CmdDeleteProc *deleteProc)
{
    ...
    hPtr = Tcl_CreateHashEntry(&iPtr->commandTable, cmdName, &newnew);
    if (!newnew) {
	/*
...
int
Tcl_Eval(Tcl_Interp *interp, char *cmd, int flags, char **termPtr)
{
    /*
...
#   define NUM_ARGS 10
    ...
    *termPtr = src;
	...
	while (1) {
	    ...
	    if ((CHAR_TYPE(((int) c)) != TCL_SPACE) && (c != ';') && (c != '\n')) {
		break;
	    }
	    src += 1;
	}
	...
	if ((result != TCL_OK) && (result != TCL_ERROR)) {
	    Tcl_ResetResult(interp);
	    if (result == TCL_BREAK) {
              sprintf (iPtr->result, "invoked \"break\" outside of a loop");
	    } else if (result == TCL_CONTINUE) {
              sprintf (iPtr->result, "invoked \"continue\" outside of a loop");
	    } else {
		iPtr->result = iPtr->resultSpace;
...
void
Tcl_AddErrorInfo(Tcl_Interp *interp, const char *message)
{
    register Interp *iPtr = (Interp *) interp;
...
#define FIXED_SIZE 200
	...
	if ((spaceUsed + length) > spaceAvl) {
	    ...
	    spaceAvl += spaceAvl/2;
	    newnew = ckalloc((unsigned) spaceAvl);
	    memcpy((VOID *) newnew, (VOID *) cmd, spaceUsed);
	    if (cmd != fixedSpace) {
		ckfree(cmd);
	    }
	    cmd = ((char *) newnew);
...
int
Tcl_GlobalEval(Tcl_Interp *interp, char *command)
{
    register Interp *iPtr = (Interp *) interp;
    int result;
