 ...
 *
 * $Id: tclParse.c,v 1.1.1.1 2001/04/20 15:03:06 karl Exp $
 */
...
char
Tcl_Backslash(const char *src, int *readPtr)
{
...
int
TclParseQuotes(Tcl_Interp *interp, char *string, int termChar, int flags, 
               char **termPtr, ParseValue *pvPtr)
{
	...
	if (c == termChar) {
	    ...
	    *termPtr = src;
	    return TCL_OK;
	} else if (CHAR_TYPE(((int) c)) == TCL_NORMAL) {
	    copy:
	    *dst = c;
	    dst++;
	    continue;
...
int
TclParseNestedCmd(Tcl_Interp *interp, char *string, int flags, 
                  char **termPtr, register ParseValue *pvPtr)
{
...
int
TclParseBraces(Tcl_Interp *interp, char *string, char **termPtr, 
               register ParseValue *pvPtr)
{
    int level;
...
int
TclParseWords(Tcl_Interp *interp, char *string, int flags, int maxWords, 
...
void
TclExpandParseValue(register ParseValue *pvPtr, int needed)
{
    int newSpace;
    char *newnew;
    ...
    newnew = (char *) ckalloc((unsigned) newSpace);
    ...
    memcpy((VOID *) newnew, (VOID *) pvPtr->buffer, pvPtr->next - pvPtr->buffer);
    pvPtr->next = newnew + (pvPtr->next - pvPtr->buffer);
    if (pvPtr->clientData != 0) {
	ckfree(pvPtr->buffer);
    }
    pvPtr->buffer = newnew;
    pvPtr->end = newnew + newSpace - 1;
...
char *
TclWordEnd(char *start, int nested)
{
    register char *p;
    int count;
...
char *
Tcl_ParseVar(Tcl_Interp *interp, register char *string, char **termPtr)
{
